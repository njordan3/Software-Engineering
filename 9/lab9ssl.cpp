...
//lab9sslget.cpp
//
//Modified By: Juan Trigueros and Nicholas Jordan
//Date: 02-07-2019
//
...
//
//Build:
//  gcc -olab9sslget lab9sslget.cpp -lssl -lcrypto
//Usage:
//  ./lab9sslget <host> <page>
//
#include <stdio.h>
#include <iostream>
#include <string.h>
...
void get_a_page(SSL *ssl, char *hostname, char *pagename, int level);
...
int main (int argc, char *argv[]) 
{
    ...
    set_non_blocking(sd);
    get_a_page(ssl, hostname, pagename, 1);
    //Cleanup.
    SSL_free(ssl);
    close(sd);
    SSL_CTX_free(ctx);
...
void get_a_page(SSL *ssl, char *hostname, char *pagename, int level)
{
    ...
    nerrs = 0;
    //key is what we are looking for in the unit test
    char key[] = "Both programmers should have the files";
    while (bytes >= 0 && nerrs < MAX_READ_ERRORS) {
        //search for the key in buf
        if (strstr(buf, key))
           unit_test = true;
    ...
    //recursion
    if (level < 2) {
        strcpy(hostname, "odin.cs.csub.edu");
        strcpy(pagename, "/~gordon/3350/lab8.html");
        get_a_page(ssl, hostname, pagename, level+1);
    }
}
